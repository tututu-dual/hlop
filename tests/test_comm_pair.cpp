...
int main(int argc, char const *argv[]) {
	...
	auto node2 = std::make_shared<hlop::df_node>("i10r5n04");
	...
	INFO("level between rank1 and rank2 is {}", *node1 - *node2);
	INFO("level between rank1 and rank3 is {}", node2->get_core_level(1, 3));
	INFO("level between rank1 and rank13 is {}", node2->get_core_level(1, 13));
	INFO("level between rank1 and rank31 is {}", node2->get_core_level(1, 31));
	INFO("node3 is {}", node3);
	...
	hlop::comm_pair_t p4_0{node1, 0,  // node 1, rank 0, core 0
	                       node1, 2}; // node 1, rank 4, core 1
	hlop::comm_pair_t p4_1{node1, 4,  // node 1, rank 2, core 2
	                       node1, 6}; // node 1, rank 6, core 3
	hlop::comm_pair_t p4_2{node1, 8,  // node 1, rank 8, core 4
	                       node1, 10}; // node 1, rank 8, core 5
	...
	INFO("p1 is {}", (p1.is_mid_pair() ? "mid" : "not mid"));
	...
	INFO("p4 is {}", (p4.is_mid_pair() ? "mid" : "not mid"));
	...
	INFO("p4_0 {} p4_2", (p4_0 == p4_2 ? "==" : "!=")); // should be !=
	...
	INFO("p8 is {}", (p8.is_mid_pair() ? "mid" : "not mid"));
